...
- **ЭТАП 8.2 / группа P1 — PDF-чеки: OCR + превью (НОВАЯ СЕССИЯ).** По плану `PLAN.md →
  Фаза 8, 8.2` (edit.md #8). Подтверждено в боевых логах: `PIL.UnidentifiedImageError:
  cannot identify image file ...pdf` (OCR не умеет PDF). Спроектировать: либа
  **pypdfium2** (pip, без системных зависимостей, кроссплат); в `ocr/recognize.py` если
  файл .pdf (или сигнатура `%PDF`) → рендер 1-й страницы в PIL.Image (scale ~2-3) → далее
  существующий Tesseract-пайплайн. Превью PDF в `purchase_detail.html` (`<embed>/<iframe>`
  на роут receipt с media_type application/pdf по расширению). Скилы: fastapi-patterns,
  test-driven-development, agent-browser (превью). Агент: coder (sonnet).
