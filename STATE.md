...
- **ЭТАП 8.13 / P4 — смена ссылки/режима таблицы у идущего мероприятия + миграция (НОВАЯ СЕССИЯ).**
  По плану `PLAN.md → Фаза 8, 8.13` (полное ТЗ уже спроектировано, развилки закрыты). Опора:
  `sync_event_to_sheet` (готовый rebuild из БД) + `raise_on_error` для синхронной валидации в
  форме + ветвление в `update_event`. Затем 8.14 (картинки к сообщениям, миграция 0009), потом
  8.12 (финальная приёмка), потом Фаза 9 (overselling).
- **8.11 ЗАВЕРШЁН И ЗАДЕПЛОЕН** (см. «Текущая точка»; деплой-строка дописана после раската).
- **ЭТАП 8.11 (поиск в «Участниках») — выполнен этой сессией.**
