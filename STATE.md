...
- **ЭТАП 8.12 / финальный деплой + приёмка (НОВАЯ СЕССИЯ).** По плану `PLAN.md → Фаза 8, 8.12`:
  полный `pytest` зелёный, alembic head на боевом, сквозная smoke + agent-browser проверка
  ключевых экранов в обеих темах, передача заказчику для ручного прогона бот-сценария в ВК
  (в т.ч. живая проверка: картинки к сообщениям доходят участнику; смена ссылки таблицы у
  идущего мероприятия). Затем **Фаза 9 (overselling/контроль лимита номеров)** — последний
  крупный блок.
