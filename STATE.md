...
- **ЭТАП 8.14 / P4 — картинки-вложения к 3 сообщениям бота (НОВАЯ СЕССИЯ).**
  По плану `PLAN.md → Фаза 8, 8.14` (полное ТЗ спроектировано). Сообщения receipt_received/
  contacts_saved/after_payment; «Инструкцию» НЕ трогаем (там QR). Механика = копия QR: отдельная
  таблица `EventMessageImage` (миграция **0009**), `settings.message_images_dir`, обобщённый
  `resolve_message_attachment`, отправка через выделенный `upload_api` (его НАДО протащить в
  воркер — он шлёт after_payment; колбэк `send_message` расширить `attachment`). UI у каждого
  сообщения: прикрепить/заменить/удалить + превью; при замене/удалении старый файл стирается с
  диска; при удалении события — файлы чистятся явно. Затем 8.12 (финальная приёмка), Фаза 9.
- **8.13 ЗАВЕРШЁН И ЗАДЕПЛОЕН** этой сессией (см. «Текущая точка»).
- **8.11 ЗАВЕРШЁН И ЗАДЕПЛОЕН** (коммит `cba3bc0`).
