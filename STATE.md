...
- **ЭТАП 8.8 / группа P3 — кнопка «Добавить вручную» в модерации (НОВАЯ СЕССИЯ).**
  По плану `PLAN.md → Фаза 8, 8.8` (edit.md #13). Роут+форма `/moderation/manual` (GET форма,
  POST создание): поля vk_link, ФИО (provided_name), phone, public_name, amount, event_id
  (выбор мероприятия), файл чека (UploadFile → receipts_dir). Логика: распарсить vk_user_id
  из ссылки, иначе синтетический отрицательный id (уникальность (event_id, vk_user_id));
  upsert участника; создать Purchase(status=manual_review, amount, receipt_file_path,
  posters_count=floor). Дальше обычные одобрить/отклонить (одобрение → воркер присвоит номера).
  КП: ручной чек создаётся, одобряется, номера присваиваются, попадает в таблицу.
