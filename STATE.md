...
- **ЭТАП 8.9 / группа P4 — модерация-канбан: 3 колонки + фильтр + «Списком» + флаг (НОВАЯ СЕССИЯ).**
  По плану `PLAN.md → Фаза 8, 8.9` (edit.md #11, #19). Два подэтапа:
  - **8.9.1 Бэкенд:** роут модерации с режимом `view=board|list` (дефолт board); board группирует
    покупки в 3 ведра — «На проверке» (pending_ocr+manual_review), «Подтверждено» (approved),
    «Отклонено» (rejected+revoked); выпадающий список мероприятий (дефолт «Все», event_id=None);
    флаг `needs_attention` → красный «!» в карточке колонки «На проверке»; лимит/пагинация на
    колонку (≈50 + «ещё»). Кнопка «Списком» сохраняет выбор мероприятия. Агент: coder (sonnet).
  - **8.9.2 Фронт:** `moderation_board.html` — 3 колонки (канбан), карточки с ключевой инфо +
    красный флаг, ссылка в карточку покупки, переключатель board/list. Обе темы, сканируемо.
    Скилы: design-taste-frontend + frontend-design-direction + agent-browser. Вёрстку канбана —
    architect (opus) или coder со скилами дизайна.
  NB: это бОльший этап (бэк+нетривиальная вёрстка) — возможно делать в 2 захода. Уточнений у
  заказчика, скорее всего, не нужно (спека в edit.md #11/#19 конкретна), но если всплывут
  развилки по виду карточек/пагинации — спросить ДО кода.
