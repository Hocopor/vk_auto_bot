...
- **2026-06-27. ФАЗА 8 — большой фидбек заказчика (edit.md, 13 пунктов). ИДЁТ.**
  ...
  - **Уточнения заказчика (зафиксированы в PLAN.md, Фаза 8, шапка):** публичное имя =
    ...
    присваивает номера).
  - **ЭТАП 8.0 (баг #2: QR/картинка не всегда отправляется) — ИДЁТ (сабагент пишет код).**
    ...
    **Осталось в этой сессии:** дождаться сабагента → прогнать pytest → e2e agent-browser
    (бейдж/предупреждение QR в админке) → деплой (push → git pull /opt+/root → alembic
    upgrade head → restart) → проверка (логи: после 1-й успешной загрузки qr_attachment
    кэшируется, ошибок нет; финальный ручной прогон в ВК — заказчик) → финал STATE/PLAN.
...
- **Фаза 7.1 (QR) исправлена в коде, тесты зелёные. Ждём push пользователя в GitHub,
  затем деплой (git pull /opt+/root → restart → smoke → UPDATE events send_qr для id=4
  → проверка отправки QR ботом). После — Фаза 7.2 (PDF-чеки), потом снова деплой.**
