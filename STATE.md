...
- **ЭТАП 8.1 / группа P1 — платёжный конвейер (НОВАЯ СЕССИЯ).** По плану `PLAN.md →
  Фаза 8, P1`. Правило заказчика: 1 этап = 1 сессия. Следующий этап — **8.1 дубль
  кодового слова → дружелюбная ошибка** (edit.md #1; в `events.py` ловить нарушение
  unique-индекса `uq_event_keyword_active`, отдавать форму с сообщением «слово занято
  мероприятием X», не 500). Затем 8.2 PDF-чеки (подтверждено в логах:
  `PIL.UnidentifiedImageError` на .pdf), затем 8.3 автоподтверждение+abuse. Деплой —
  инкрементально по группам. Оркестратор проектирует → coder-сабагент пишет (скилы
  test-driven-development + fastapi-patterns), e2e agent-browser, потом деплой.
