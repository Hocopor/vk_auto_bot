...
- **2026-06-28. ФАЗА 9 (контроль лимита номеров / overselling + стоп-бота по исчерпанию) —
  КОД+ТЕСТЫ ГОТОВЫ; ДЕПЛОЙ ЖДЁТ SSH_PASS.** Реализацию делал ОРКЕСТРАТОР сам (критичное ядро
...
- **ДЕПЛОЙ ФАЗЫ 9 (когда будет `SSH_PASS`) — ПЕРВЫМ ДЕЛОМ В НОВОЙ СЕССИИ.** Коннект по `ssh.md`
  (root@185.228.72.118, paramiko `scripts/_ssh_run.py`, пароль в env `SSH_PASS`): push (если не
  сделан) → `git pull --ff-only` /opt+/root → **`alembic upgrade head` (миграция 0010
  `numbers_shortfall` — ОБЯЗАТЕЛЬНО, иначе боевая БД упадёт)** → restart admin-web+vk-bot →
  smoke `:8080/login=200`. После деплоя в логе vk-bot ДОЛЖЕН пропасть спам `Numbers exhausted
  for event …` (баг overselling закрыт).
